...
class TestSystemStructure:
    ...
    def test_hooks_structure(self):
        ...
        required_hooks = ["hooks/README.md", "hooks/pre-run.md", "hooks/post-run.md"]
...
class TestClaudeJSON:
    ...
    def test_agents_section(self, claude_config):
        ...
        for agent_name, agent_config in agents.items():
            ...
            assert isinstance(
                agent_config["priority"], int
            ), f"Agent {agent_name} priority must be an integer"
            assert 1 <= agent_config["priority"] <= 3, f"Agent {agent_name} priority must be 1-3"
    ...
    def test_governance_section(self, claude_config):
        ...
        assert (
            governance.get("hooks_enabled") is True
        ...
        for validator in expected_validators:
            assert validator in validators, f"Required validator missing: {validator}"
    ...
    def test_skills_integration(self, claude_config):
        ...
        assert skills.get("enabled") is True, "Skills integration should be enabled"

        available_skills = skills.get("available_skills", [])
        expected_skills = ["docx", "xlsx", "pptx", "pdf"]

        for skill in expected_skills:
            assert skill in available_skills, f"Expected skill missing: {skill}"
...
class TestAgents:
    ...
    def test_all_agents_have_files(self, claude_config):
        ...
        for agent_name, agent_config in agents.items():
            ...
            assert agent_file.exists(), f"Agent file missing for {agent_name}: {agent_file}"
            assert agent_file.is_file(), f"Agent path exists but is not a file: {agent_file}"
    ...
    def test_agent_files_not_empty(self, claude_config):
        ...
        for agent_name, agent_config in agents.items():
            ...
            assert len(content) > 100, f"Agent file {agent_name} seems too short (< 100 chars)"
    ...
    def test_agent_files_have_required_sections(self, claude_config):
        ...
        for agent_name, agent_config in agents.items():
            ...
            for section in required_sections:
                assert section in content, f"Agent {agent_name} missing required section: {section}"
...
class TestContracts:
    ...
    def test_contract_files_have_required_sections(self, claude_config):
        ...
        for agent_name, agent_config in agents.items():
            ...
            for section in required_sections:
                assert section in content, f"Contract for {agent_name} missing section: {section}"
...
class TestValidators:
    ...
    def test_all_validators_exist(self, claude_config):
        ...
        for validator_name in validators:
            ...
            assert validator_file.exists(), f"Validator file missing: {validator_name}"
    ...
    def test_validator_files_have_structure(self, claude_config):
        ...
        for validator_name in validators:
            ...
            for section in required_sections:
                assert section in content, f"Validator {validator_name} missing section: {section}"
...
class TestSkillsIntegration:
    ...
    def test_skills_readme_documents_all_skills(self):
        ...
        for skill in expected_skills:
            assert skill in content, f"Skills README doesn't document {skill} skill"
    ...
    def test_skills_readme_has_integration_patterns(self):
        ...
        assert "Integration Pattern" in content, "Skills README missing integration patterns"
        assert "file_read" in content, "Skills README doesn't show how to read skill documentation"
...
class TestSystemIntegrity:
    ...
    def test_no_orphaned_agent_files(self, claude_config):
        ...
        defined_agents = set(
            Path(cfg["file"]).name for cfg in claude_config.get("agents", {}).values()
        )

        actual_agent_files = set(f.name for f in agents_dir.glob("*.md"))

        orphaned = actual_agent_files - defined_agents
        assert len(orphaned) == 0, f"Orphaned agent files found (not in claude.json): {orphaned}"
    ...
    def test_no_orphaned_contract_files(self, claude_config):
        ...
        defined_contracts = set(
            Path(cfg["contract"]).name for cfg in claude_config.get("agents", {}).values()
        )

        actual_contract_files = set(f.name for f in contracts_dir.glob("*.contract"))

        orphaned = actual_contract_files - defined_contracts
        assert len(orphaned) == 0, f"Orphaned contract files found: {orphaned}"
...
class TestDocumentation:
    ...
    def test_readme_is_comprehensive(self):
        ...
        expected_sections = ["Purpose", "Directory Structure", "Quick Start", "Agents", "Workflows"]

        for section in expected_sections:
            assert section in content, f"README missing section about: {section}"
