...
class TestHybridOrchestrator(unittest.TestCase):
    ...
    def test_select_model_manual_override(self, mock_init):
        ...
        # Override to simple
        model = orchestrator.select_model_for_agent(
            "frontend-architect",  # Normally Sonnet
            "Some task",
        ...
        # Override to complex
        model = orchestrator.select_model_for_agent(
            "document-writer-expert",  # Normally Haiku
            "Some task",
    ...
    def test_estimate_cost_varies_by_complexity(self, mock_init):
        ...
        # Simple task
        simple_estimate = orchestrator.estimate_cost(
            "Fix typo",
            "test-agent",
            model=model
        )
        ...
        self.assertGreater(
            complex_estimate.estimated_output_tokens,
            simple_estimate.estimated_output_tokens
        )
